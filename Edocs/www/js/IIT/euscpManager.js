...
var URL_GET_CERTIFICATES = "/Content/CACertificates.p7b?version=1.0.14";
var URL_CAS = "/Content/CAs.json?version=1.0.14";
//var URL_XML_HTTP_PROXY_SERVICE = "/ProxyHandler.ashx";
var URL_XML_HTTP_PROXY_SERVICE = 'https://qa-online.e-docs.ua' + "/ProxyHandler.ashx";
var IIT_TEST_SERVER = 'Тестовий ЦСК АТ "ІІТ"';
var ACSK_PRIVAT = 'acsk.privatbank.ua';
var ACSK_INTERLINE = 'a.altersign.com.ua';
var ACSK_UKRAINE = 'uakey.com.ua';

//var DOMAIN = "http://192.168.137.1:3000";
var DOMAIN = "http://192.168.1.117:3000";

function initValues() {
    if (device.platform === 'Android') {
        URL_GET_CERTIFICATES = DOMAIN + URL_GET_CERTIFICATES;
        URL_CAS = DOMAIN + URL_CAS;
        //URL_XML_HTTP_PROXY_SERVICE = 'http://localhost:52054' + URL_XML_HTTP_PROXY_SERVICE;
    }
}
    ...
    },
    {
        ...
        readPrivateKeyButtonClick: function () {
                ...
                function checkPermissionCallback(status) {
                    if (!status.hasPermission) {
                        var errorCallback = function () {
                            alert('Для вибору файлу необхідно дати доступ до зчитування файлів.');
                        }
            ...
            function readPrivateKey() {
                try {
                    var cert = document.getElementById('ChoosePKCertsInput').files;
                    euSignTest.privateKeyCerts = cert;
                    ...
                    var _onSuccess = function (keyName, key) {
                        euSignTest.readPrivateKey(keyName,
                            new Uint8Array(key),
                    ...
                    try {
                        if (document.getElementById('PKeyReadButton').title == '') {
                            setStatus('зчитування ключа');
        ...
        showOwnerLabel: function () {
            try {
                var ownerInfo = euSign.GetPrivateKeyOwnerInfo();
                    document.getElementById('owner-info').style.display = 'block';
                    document.getElementById('owner-info-label').innerHTML = "Власник: " + ownerInfo.GetSubjCN() + "<br />" +
        ...
        signFile: function () {
            ...
            //var hashSign = euSign.SignHash('P3FIrjwoAxsD4SrnXyCfZNRAIcS5rHBlO92pbbNYla4=', false);
            //var binarySign = new Uint8Array(hashSign);
            ...
            fileReader.onloadend = (function (fileName) {
                return function (evt) {
                    ...
                    try {
                        ...
                        saveFile(fileName + ".p7s", sign);
                        saveFile(fileName + 'hash.p7s', hashSign);
        ...
        privateKeyReaded: function (isReaded) {
            ...
            //document.getElementById('SignFileButton').disabled = enabled;
...
$('#CAsServersSelect').on('change', function (e) {
    debugger;
    var caServer = this.value;
    if (caServer === IIT_TEST_SERVER) {
        $('#PKCertsSelectZone').show();
    } else {
        $('#PKCertsSelectZone').hide();
    }
});
...
$("#PKeyFileInput").change(function () {
    if (this.value) {
        ...
        switch (ext) {
            case 'jks':
                index = euSignTest.CAsServers.findIndex(x => x.address == ACSK_PRIVAT);
                break;
            case 'pk8':
                index = euSignTest.CAsServers.findIndex(x => x.address == ACSK_INTERLINE);
                break;
            case 'zs2':
                index = euSignTest.CAsServers.findIndex(x => x.address == ACSK_UKRAINE);
                break;
            case 'sk':
                index = euSignTest.CAsServers.findIndex(x => x.address == ACSK_UKRAINE);
                break;
        ...
        if (index) {
            $('#CAsServersSelect').prop('disabled', true);
            $('#CAsServersSelect option').eq(index).prop('selected', true);
        }
    }
});
...
function onDeviceReady() {
    ...
    if (needToInitialize) {
        EUSignCPModuleInitialized(true);
    }
}
