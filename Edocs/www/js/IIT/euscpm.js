		...
		if (useASyncAPI) {
			...
			function _onSuccess(evt) {
				...
				try {
				    ...
				    alert('Load file error1 (ReadFile): ' + e);
					onError(pThis.MakeError(EU_ERROR_JS_READ_FILE));
				}
			};

            function _onError(e) {
                alert('Load file error2 (ReadFile): ' + e);
				onError(pThis.MakeError(EU_ERROR_JS_READ_FILE));
			}
...
//-----------------------------------------------------------------------------
	...
	GetSignTimeInfo: function(signIndex, sign) {
		...
		timeInfo = new EndUserTimeInfo(timeInfoPtr + 4);
